...
function FloatingActionItem(props) {
  ...
  const { active, animated, distanceToEdge, distanceToEdge, name, onPress } = props;
  const {
  ...
  const { buttonSize, icon, color, shadow, tintColor } = props;
  const {
    position,
    paddingTopBottom,
    render,
  ...
  const [animation, setAnimation] = useState(Animated.Value(0));
  const [activeInital, setActiveInitial] = useState(active);

  useEffect(() => {
    if (active !== activeInital && animated) {
